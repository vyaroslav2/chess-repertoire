...
- the stored RESPONSE
- Local Deep Stockfish's best move from the same position

A valid existing `LocalDeepEvalCache` result for that exact Position + RESPONSE may be reused.
...
- obtaining or reusing Local Deep Stockfish analysis
...
The deep-engine cache is responsible for storing reusable Local Deep Stockfish analysis.
